...
public class GetAreaFunction implements InterlisFunction {
    ...
    public Value evaluate(String validationKind, String usageScope, IomObject contextObject, Value[] arguments) {
        ...
        if (argPath.isUndefined()) {
            ...
            PathEl[] attributePath = getAttributePathEl(contextClass, argPath);
            ...
            for (IomObject rootObject : argObjects.getComplexObjects()) {
                ...
                if (!(surfaceAttributes.isUndefined() || surfaceAttributes.skipEvaluation() || surfaceAttributes.getComplexObjects() == null)) {
                    surfaces.addAll(surfaceAttributes.getComplexObjects());
                }
            }
        }
        ...
        for (IomObject surface : surfaces) {
            try {
                result += Iox2jtsext.surface2JTS(surface, 0.0).getArea();
            } catch (IoxException ex) {
                logger.addEvent(logger.logErrorMsg("Could not calculate GetArea for Object {0}", surface.toString()));
            }
        }
    ...
    private PathEl[] getAttributePathEl(Viewable<Element> contextClass, Value argPath) {
        try {
